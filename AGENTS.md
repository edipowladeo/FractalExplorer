...
## Worktrees e caches compartilhados

- Criar novos worktrees em `C:\Users\edipo\repo\fractalrenderers\FractalExplorer-worktrees`.
- Usar `git worktree add "$env:FRACTAL_EXPLORER_WORKTREES\<nome>" <branch>`.
- `CARGO_TARGET_DIR` aponta para `C:\Users\edipo\repo\fractalrenderers\.cargo-target`.
- `GRADLE_USER_HOME` aponta para `C:\Users\edipo\repo\fractalrenderers\.gradle-user`.
- Manter `target/` e `build/` específicos por projeto/worktree; compartilhar apenas caches e dependências.
